...
public class PrePostProcessor {
    // for yolov7 model, no need to apply MEAN and STD
    static float[] NO_MEAN_RGB = new float[] {0.0f, 0.0f, 0.0f};
    static float[] NO_STD_RGB = new float[] {1.0f, 1.0f, 1.0f};

    // model input image size
    static int mInputWidth = 320; // 640
    static int mInputHeight = 320; // 640

    // model output is of size 25200*(num_of_class+5)
    private static int mOutputRow = 6300; // as decided by the YOLOv5 model for input image of size 640*640 -> 25200, 320*320 -> 6300
    private static int mOutputColumn = 6; // left, top, right, bottom, score and N class probabilities
    private static float mThreshold = 0.10f; // score above which a detection is generated
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< mOutputRow; i++) { // iterate over grid
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                ...
                Result result = new Result(cls, outputs[i*mOutputColumn+4], rect); // class, confidence, (x, y, w, h)
                results.add(result);
            }
        }
